...
int main(){
    ...
    AMGX_SAFE_CALL(AMGX_config_create(&cfg, "solver=JACOBI, max_iters=1, relaxation_factor=0.5, ilu_sparsity_level=0, insert_diag_while_reordering=1, monitor_residual=1, print_solve_stats=1"));
    ...
    int num_rows = 4;  // Adjust as needed
    ...
    AMGX_SAFE_CALL(AMGX_vector_upload(x, num_rows, 1, input_vector_x.data()));
    AMGX_SAFE_CALL(AMGX_vector_upload(b, num_rows, 1, input_vector_b.data()));
    ...
    AMGX_SAFE_CALL(AMGX_matrix_upload_all(A, num_rows, 8, 1, 1, row_ptrs.data(), col_indices.data(), values.data(), NULL));
