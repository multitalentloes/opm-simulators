...
#define BOOST_TEST_MODULE TestCuAD
...
BOOST_AUTO_TEST_CASE(TestInstansiateADObject)
{
    ...
    OPM_CUDA_SAFE_CALL(cudaMalloc(&d_ad, sizeof(Evaluation)));
    instansiate_ad_object<<<1,1>>>(d_ad, testValue);
    OPM_CUDA_SAFE_CALL(cudaDeviceSynchronize());
    OPM_CUDA_SAFE_CALL(cudaMemcpy(&gpuMadeAd, d_ad, sizeof(Evaluation), cudaMemcpyDeviceToHost));
