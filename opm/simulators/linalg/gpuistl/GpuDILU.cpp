...
public:
    ...
    DILUAPPLY() : start_time(std::chrono::high_resolution_clock::now()) {
        ++instance_count;
    }
...
long long DILUAPPLY::total_time_spent = 0;
int DILUAPPLY::instance_count = 0;

class DILUUPDATE {
public:
    // Constructor starts the timer
    DILUUPDATE() : start_time(std::chrono::high_resolution_clock::now()) {
        ++instance_count;
    }
...
// Static member variables need to be defined outside the class
long long DILUUPDATE::total_time_spent = 0;
int DILUUPDATE::instance_count = 0;
